'use client';

import { useState, useEffect, useRef } from 'react';
import Header from '../components/header';
import ProductCard from '../components/product-card/product-card';
import Image from 'next/image';

const ITEM = {
  productLink: '',
  photoLink: '',
  photoFile: null as File | null,
  productName: '',
  priceWithoutDiscount: '',
  priceWithDiscount: ''
};

export default function Home() {

  const [ siteLink, setSiteLink ] = useState( '' );
  const [ shop, setShop ] = useState( '' );
  const [ minimalCartPrice, setMinimalCartPrice ] = useState( '' );
  const [ minPrice, setMinPrice ] = useState( '' );
  const [ maxPrice, setMaxPrice ] = useState( '' );
  const [ count, setCount ] = useState( '' );
  const [ commision, setCommision ] = useState( 1.1 );
  const [ currency, setCurrency ] = useState( 'USD' );

  const [ shops, setShops ] = useState( [] );
  const [ selectedShop, setSelectedShop ] = useState( '' );

  const [ isLoading, setIsLoading ] = useState( false );

  const [ previewItems, setPreviewItems ] = useState<any[] | null>( null );
  const [ addItemMode, setAddItemMode ] = useState( false );

  const [ lastJobId, setLastJobId ] = useState<string | null>( null );
  const [ jobStatus, setJobStatus ] = useState<'processing' | 'completed' | 'failed' | null>( null );
  const [ jobError, setJobError ] = useState<string | null>( null );
  const pollingAbortRef = useRef( false );

  // Add item form state
  const [ addItemFormData, setAddItemFormData ] = useState( ITEM );

  useEffect( () => {
    fetch( '/api/shops' )
      .then( res => res.ok ? res.json() : [] )
      .then( setShops )
      .catch( console.error );
  }, [] );

  const sleep = ( ms: number ) => new Promise( resolve => setTimeout( resolve, ms ) );

  const clearSavedJobId = () => {
    pollingAbortRef.current = false;

    if ( typeof window !== 'undefined' ) {
      localStorage.removeItem( 'lastJobId' );
    }

    setLastJobId( null );
  };

  const abortPolling = () => {
    pollingAbortRef.current = true;
    setJobStatus( 'failed' );
    setJobError( 'Polling aborted by user' );
    setIsLoading( false );
    clearSavedJobId();
  };

  const pollAgentStatus = async ( id: string ) => {
    while ( true ) {
      if ( pollingAbortRef.current ) {
        return { status: 'failed', error: 'Polling aborted by user', data: null };
      }

      const statusResponse = await fetch( `/api/agentStatus?id=${ encodeURIComponent( id ) }` );
      if ( !statusResponse.ok ) {
        throw new Error( `Failed to fetch agent status: ${ statusResponse.status }` );
      }

      const statusData = await statusResponse.json();
      console.log( 'Agent Status:', statusData );

      if ( statusData.status === 'processing' ) {
        await sleep( 5000 );
        continue;
      }

      return statusData;
    }
  };

  const continueJobStatus = async ( id: string ) => {
    pollingAbortRef.current = false;
    setLastJobId( id );
    setJobStatus( 'processing' );
    setJobError( null );
    setIsLoading( true );

    try {
      const finalStatus = await pollAgentStatus( id );

      setJobStatus( finalStatus.status );

      if ( finalStatus.status === 'completed' ) {
        setPreviewItems( finalStatus.data.products );
        // clearSavedJobId();
      } else if ( finalStatus.status === 'failed' ) {
        setJobError( finalStatus.error || 'Agent failed' );
        // clearSavedJobId();
      }
    } catch ( error: any ) {
      setJobStatus( 'failed' );
      setJobError( error.message || 'Agent status polling failed' );
      // clearSavedJobId();
    } finally {
      setIsLoading( false );
    }
  };

  useEffect( () => {
    if ( typeof window === 'undefined' ) return;
    const savedId = localStorage.getItem( 'lastJobId' );
    if ( savedId ) {
      continueJobStatus( savedId );
    }
  }, [] );

  const submitForm = async ( e: React.FormEvent<HTMLFormElement> ) => {
    e.preventDefault();

    if ( !siteLink || !selectedShop || !minimalCartPrice ) {
      alert( 'Please fill in all required fields' );
      return;
    }

    setIsLoading( true );
    // v1

    // try {
    //   const response = await fetch( `/api/discounts?link=${ encodeURIComponent( siteLink ) }&shop=${ encodeURIComponent( selectedShop ) }&minimalCartPrice=${ encodeURIComponent( minimalCartPrice ) }&minPrice=${ encodeURIComponent( minPrice ) }&maxPrice=${ encodeURIComponent( maxPrice ) }&count=${ encodeURIComponent( count ) }&commision=${ encodeURIComponent( commision.toString() ) }&currency=${ encodeURIComponent( currency ) }` );

    //   if ( response.ok ) {
    //     const data = await response.json();
    //     console.log( 'API Response:', data );
    //   } else {
    //     console.error( 'API Error:', response.statusText );
    //   }
    // } catch ( error ) {
    //   console.error( 'Fetch Error:', error );
    // } finally {
    //   setIsLoading( false );
    // }

    // --------------------------------------------------------------------------
    // v2

    try {
      const response = await fetch( `/api/discounts-v2?link=${ encodeURIComponent( siteLink ) }&shop=${ encodeURIComponent( selectedShop ) }&minimalCartPrice=${ encodeURIComponent( minimalCartPrice ) }&minPrice=${ encodeURIComponent( minPrice ) }&maxPrice=${ encodeURIComponent( maxPrice ) }&count=${ encodeURIComponent( count ) }&commision=${ encodeURIComponent( commision.toString() ) }&currency=${ encodeURIComponent( currency ) }` );

      if ( response.ok ) {
        const data: {
          success: boolean;
          id: string;
          error?: string;
        } = await response.json();

        if ( data.id ) {
          if ( typeof window !== 'undefined' ) {
            localStorage.setItem( 'lastJobId', data.id );
          }
          await continueJobStatus( data.id );
        }
      } else {
        console.error( 'API Error:', response.statusText );
      }
    } catch ( error ) {
      console.error( 'Fetch Error:', error );
    } finally {
      setIsLoading( false );
    }

    // try {
    //   const finalStatus = await pollAgentStatus( '019e34f4-6e13-76ee-936c-e4252b1570d1' );

    //   if ( finalStatus.status === 'completed' ) {
    //     console.log( 'Agent completed successfully.', finalStatus.data );
    //     setIsLoading( false );
    //     setPreviewItems( finalStatus.data.products );
    //   } else if ( finalStatus.status === 'failed' ) {
    //     console.error( 'Agent failed.', finalStatus.error );
    //   }
    // } catch ( statusError ) {
    //   console.error( 'Agent status polling failed:', statusError );
    // }
  }

  const removeItem = ( idx: number ) => {
    setPreviewItems( prev => prev ? prev.filter( ( _, i ) => i !== idx ) : prev );
  };

  const saveItems = async () => {
    if ( !previewItems || previewItems.length === 0 ) {
      alert( 'Немає товарів для збереження' );
      return;
    }

    setIsLoading( true );

    try {
      const response = await fetch( '/api/save-sale', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify( {
          previewItems,
          shop: selectedShop,
          minimalCartPrice: parseFloat( minimalCartPrice ),
          commission: commision,
          currency
        } )
      } );

      if ( response.ok ) {
        const data = await response.json();
        console.log( 'Sale saved successfully:', data );
        alert( 'Товари успішно збережені' );
        setPreviewItems( null );
        setSiteLink( '' );
        setSelectedShop( '' );
        setMinimalCartPrice( '' );
        setMinPrice( '' );
        setMaxPrice( '' );
        setCount( '' );
        setCommision( 1.1 );
        setCurrency( 'USD' );

        clearSavedJobId();
      } else {
        const errorData = await response.json();
        console.error( 'Error saving sale:', errorData );
        alert( `Помилка при збереженні: ${ errorData.error || 'Unknown error' }` );
      }
    } catch ( error ) {
      console.error( 'Error:', error );
      alert( 'Помилка при збереженні товарів' );
    } finally {
      setIsLoading( false );
    }
  }

  async function additem(): Promise<void> {
    let imageUrl = addItemFormData.photoLink;

    // Upload photo file if provided
    if ( addItemFormData.photoFile && !imageUrl ) {
      try {
        const formData = new FormData();
        formData.append( 'file', addItemFormData.photoFile );

        const uploadResponse = await fetch( '/api/upload', {
          method: 'POST',
          body: formData
        } );

        if ( uploadResponse.ok ) {
          const uploadData = await uploadResponse.json();
          imageUrl = uploadData.url;
        } else {
          console.error( 'Upload failed' );
          alert( 'Помилка при завантаженні фото' );
          return;
        }
      } catch ( error ) {
        console.error( 'Upload error:', error );
        alert( 'Помилка при завантаженні фото' );
        return;
      }
    }

    setPreviewItems( prev => [
      ...( prev || [] ),
      {
        name: addItemFormData.productName,
        price: addItemFormData.priceWithDiscount,
        old_price: addItemFormData.priceWithoutDiscount,
        link: addItemFormData.productLink,
        image: imageUrl || ''
      }
    ] );
    setAddItemMode( false );
    setAddItemFormData( ITEM );
  }

  const handleAddItemFormChange = ( field: string, value: any ) => {
    setAddItemFormData( prev => ( { ...prev, [ field ]: value } ) );
  };

  return (
    <div className='container-c'>
      <Header />
      <main>
        {jobStatus === 'processing' && (
          <div style={{
            position: 'fixed',
            top: 0,
            left: 0,
            width: '100vw',
            height: '100vh',
            backgroundColor: 'rgba(0, 0, 0, 0.95)',
            color: '#fff',
            display: 'flex',
            flexDirection: 'column',
            justifyContent: 'center',
            alignItems: 'center',
            zIndex: 9999,
            padding: '16px',
            textAlign: 'center',
          }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', maxWidth: '500px' }}>
              <h2>Пошук товарів</h2>
              <p style={{ margin: '16px 0' }}>Зачекайте поки триває пошук товарів...</p>
              <svg className='rotate' fill="#ffffff" version="1.1"
                width="25px" height="25px" viewBox="0 0 97.541 97.54"
              >
                <g>
                  <g>
                    <g>
                      <path d="M70.063,27.182c1.398,1.175,3.174,1.821,4.996,1.821c2.312,0,4.488-1.014,5.974-2.782
				c2.76-3.289,2.329-8.211-0.96-10.973c-1.4-1.175-3.176-1.822-5-1.822c-2.311,0-4.487,1.014-5.971,2.782
				C66.341,19.498,66.773,24.419,70.063,27.182z"/>
                      <path d="M88.445,36.096c-0.483,0-0.971,0.043-1.448,0.127c-4.485,0.791-7.493,5.086-6.702,9.573
				c0.696,3.955,4.111,6.825,8.119,6.825c0.482,0,0.972-0.043,1.451-0.126c4.485-0.792,7.492-5.086,6.701-9.571
				C95.868,38.968,92.452,36.096,88.445,36.096z"/>
                      <path d="M88.158,63.113c-1.328-0.768-2.834-1.172-4.354-1.172c-3.118,0-6.022,1.675-7.579,4.371
				c-1.165,2.019-1.477,4.371-0.872,6.625s2.052,4.139,4.069,5.304c1.329,0.769,2.835,1.174,4.357,1.174
				c3.116,0,6.02-1.674,7.576-4.369C93.761,70.874,92.327,65.521,88.158,63.113z"/>
                      <path d="M63.316,78.646c-1.07,0-2.13,0.188-3.15,0.558c-2.31,0.841-4.153,2.532-5.193,4.761c-1.039,2.229-1.148,4.729-0.308,7.04
				c1.32,3.626,4.798,6.063,8.654,6.063c1.07,0,2.13-0.188,3.147-0.559c2.308-0.841,4.15-2.531,5.191-4.764
				c1.04-2.23,1.15-4.73,0.312-7.037C70.651,81.083,67.172,78.646,63.316,78.646z"/>
                      <path d="M39.903,78.757c-1.074-0.39-2.188-0.588-3.31-0.588c-4.054,0-7.71,2.562-9.097,6.375
				c-0.886,2.431-0.771,5.06,0.322,7.403c1.092,2.344,3.031,4.121,5.462,5.006c1.072,0.391,2.187,0.587,3.312,0.587
				c4.056,0,7.711-2.562,9.097-6.372c0.884-2.426,0.768-5.055-0.326-7.4C44.268,81.42,42.33,79.641,39.903,78.757z"/>
                      <path d="M24.916,65.6c-1.81-3.133-5.183-5.078-8.805-5.078c-1.771,0-3.522,0.472-5.067,1.361c-2.35,1.357-4.03,3.549-4.731,6.166
				c-0.703,2.62-0.343,5.357,1.014,7.706c1.81,3.134,5.184,5.08,8.806,5.08c1.77,0,3.521-0.472,5.065-1.362
				C26.046,76.674,27.714,70.45,24.916,65.6z"/>
                      <path d="M11.495,54.991c5.158,0,9.555-3.695,10.453-8.786c0.492-2.797-0.133-5.617-1.762-7.94
				c-1.627-2.326-4.063-3.878-6.861-4.372c-0.62-0.108-1.247-0.163-1.86-0.163c-5.158,0-9.555,3.694-10.453,8.785
				C0.52,45.31,1.145,48.13,2.774,50.456c1.628,2.325,4.065,3.878,6.861,4.371C10.252,54.936,10.878,54.991,11.495,54.991z"/>
                      <path d="M24.849,32.319c2.599,0,5.131-0.923,7.13-2.598c2.268-1.903,3.659-4.58,3.918-7.538c0.259-2.958-0.647-5.836-2.551-8.104
				c-2.114-2.52-5.217-3.965-8.511-3.965c-2.603,0-5.135,0.922-7.131,2.597c-2.271,1.906-3.665,4.583-3.923,7.537
				c-0.259,2.952,0.648,5.831,2.555,8.104C18.453,30.873,21.555,32.319,24.849,32.319z"/>
                    </g>
                    <circle cx="49.955" cy="12.076" r="12.076" />
                  </g>
                </g>
              </svg>
              <br />
              <button
                className='site-button'
                style={{ padding: '12px 24px' }}
                onClick={abortPolling}
              >
                Припинити пошук
              </button>
            </div>
          </div>
        )}
        {/* {lastJobId && jobStatus !== 'processing' && (
          <div className='job-status'>
            <p>Job ID: {lastJobId}</p>
            <p>Status: {jobStatus ?? 'checking...'}</p>
            {jobError && <p className='error'>Error: {jobError}</p>}
          </div>
        )} */}
        {
          ( previewItems && !addItemMode ) ? (
            <div className='preview'>
              <h2>Превью товаров:</h2>
              <div className='preview-items grid'>
                {previewItems.map( ( item, index ) => (
                  <ProductCard key={index} product={item} idx={index} removeItem={removeItem} />
                ) )}
              </div>
              <div className='save-button'>
                <button className='site-button add-item-button' onClick={() => setAddItemMode( true )}>Додати</button>
                <button className='site-button ' onClick={() => saveItems()}>Зберегти</button>
              </div>
            </div>
          ) :
            (
              addItemMode ? ( <div className='preview'>
                <form className='add-item-form'>
                  <label>Посилання на товар</label>
                  <input className='input' type="text" placeholder='Посилання на товар' value={addItemFormData.productLink} onChange={( e ) => handleAddItemFormChange( 'productLink', e.target.value )} />
                  <hr />
                  <label>Фото товару</label>
                  <input className='input' type="text" placeholder='Посилання на фото' value={addItemFormData.photoLink} onChange={( e ) => handleAddItemFormChange( 'photoLink', e.target.value )} />
                  або завантажити:
                  <input className='input' type="file" onChange={( e ) => handleAddItemFormChange( 'photoFile', e.target.files?.[ 0 ] || null )} />
                  <hr />
                  <label>Назва товару</label>
                  <input className='input' type="text" placeholder='Назва товару' value={addItemFormData.productName} onChange={( e ) => handleAddItemFormChange( 'productName', e.target.value )} />
                  <label>Ціна без знижки</label>
                  <input className='input' type="text" placeholder='Ціна' value={addItemFormData.priceWithoutDiscount} onChange={( e ) => handleAddItemFormChange( 'priceWithoutDiscount', e.target.value )} />
                  <label>Ціна зі знижкою</label>
                  <input className='input' type="text" placeholder='Ціна' value={addItemFormData.priceWithDiscount} onChange={( e ) => handleAddItemFormChange( 'priceWithDiscount', e.target.value )} />
                  <div className='add-item-buttons flex justify-between'>
                    <button className='site-button' type="button" onClick={() => setAddItemMode( false )}>Назад</button>
                    <button className='site-button' type="button" onClick={() => additem()}>Додати товар</button>
                  </div>
                </form>
              </div> ) : (
                <form className='main'>
                  <label>Виберіть магазин</label>
                  <select className='input' value={selectedShop} onChange={( e ) => setSelectedShop( e.target.value )} disabled={isLoading} required>
                    <option value=''>Не обрано</option>
                    {shops.map( ( s ) => (
                      <option key={s._id} value={s._id}>{s.title}</option>
                    ) )}
                  </select>
                  <label>Введіть посилання на сторінку зі знижками</label>
                  <input className='site-input input' type="text" disabled={isLoading} name="site" placeholder='https://...' value={siteLink} onChange={( e ) => setSiteLink( e.target.value )} />
                  <label>Введіть валюту cайту</label>
                  <input className='currency-input input' type="text" disabled={isLoading} name="currency" placeholder='USD, EUR, GBP...' value={currency} onChange={( e ) => setCurrency( e.target.value.toUpperCase() )} />
                  <label>Введіть комісію</label>
                  <input className='commision-input input' type="text" disabled={isLoading} name="commision" placeholder='Комісія' value={commision} onChange={( e ) => setCommision( parseFloat( e.target.value ) )} />
                  <label>Введіть мінімальну суму кошика</label>
                  <input className='minimal-cart-price-input input' type="text" disabled={isLoading} name="minimalCartPrice" placeholder='Мінімальна сума кошика' value={minimalCartPrice} onChange={( e ) => setMinimalCartPrice( e.target.value )} />
                  <label>Введіть мінімальну ціну</label>
                  <input className='min-price-input input' type="text" name="minPrice" disabled={isLoading} placeholder='Мінімальна ціна' value={minPrice} onChange={( e ) => setMinPrice( e.target.value )} />
                  <label>Введіть максимальну ціну</label>
                  <input className='max-price-input input' type="text" name="maxPrice" disabled={isLoading} placeholder='Максимальна ціна' value={maxPrice} onChange={( e ) => setMaxPrice( e.target.value )} />
                  <label>Введіть кількість товарів</label>
                  <input className='count-input input' type="text" name="count" disabled={isLoading} placeholder='Кількість товарів' value={count} onChange={( e ) => setCount( e.target.value )} />

                  <button className='site-button' type="submit" onClick={submitForm} disabled={isLoading}>{isLoading ? 'Завантаження...' : 'Завантажити знижки'}</button>
                </form>
              )
            )
        }
      </main >
    </div >
  );
}
